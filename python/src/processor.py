...
async def mint_nft(payer_keypair: KeypairInput, mint_keypair: KeypairInput, client: AsyncClient, log_level: int = 0) -> str:
    ...
    pd_pool_pubkey, _pd_pool_pubkey_bump = PublicKey.find_program_address([bytes(ingl_constants.PD_POOL_ACCOUNT_KEY, 'UTF-8')], get_program_id())
...
async def delegate_nft(payer_keypair: KeypairInput, mint_pubkey: PubkeyInput, client: AsyncClient, log_level: int = 0) -> str:
    nft_account_pubkey, _nft_account_bump = PublicKey.find_program_address([bytes(ingl_constants.NFT_ACCOUNT_CONST, 'UTF-8'), bytes(mint_pubkey.public_key)], get_program_id())
    ...
    payer_account_meta = AccountMeta(payer_keypair.public_key, True, True)
    mint_account_meta = AccountMeta(mint_pubkey.public_key, False, False)
    nft_account_meta = AccountMeta(nft_account_pubkey, False, True)
...
async def undelegate_nft(payer_keypair: KeypairInput, mint_pubkey: PubkeyInput, client: AsyncClient, log_level: int = 0) -> str:
    ...
    config_account_pubkey, _config_account_bump = PublicKey.find_program_address([bytes(ingl_constants.INGL_CONFIG_SEED, 'UTF-8')], get_program_id())
    general_account_pubkey, _general_account_bump = PublicKey.find_program_address([bytes(ingl_constants.GENERAL_ACCOUNT_SEED, 'UTF-8')], get_program_id())
    expected_vote_pubkey, _expected_vote_bump = PublicKey.find_program_address([bytes(ingl_constants.VOTE_ACCOUNT_KEY, 'UTF-8')], get_program_id())
    ...
    vote_account_meta = AccountMeta(expected_vote_pubkey.public_key, False, False)
...
async def init_rebalance(payer_keypair: KeypairInput, client: AsyncClient, log_level: int = 0) -> str:
    ...
    validator_id = PublicKey(ValidatorConfig.parse(data.value.data).validator_id)
...
async def nft_withdraw(payer_keypair: KeypairInput, mints: List[PublicKey], client: AsyncClient, log_level: int = 0) -> str:
    ...
    vote_account_id, _vote_account_bump = PublicKey.find_program_address([bytes(ingl_constants.VOTE_ACCOUNT_KEY, 'UTF-8')], get_program_id())
    general_account_pubkey, _general_account_bump = PublicKey.find_program_address([bytes(ingl_constants.GENERAL_ACCOUNT_SEED, 'UTF-8')], get_program_id())
...
async def init_governance(payer_keypair: KeypairInput, mint: PubkeyInput, client: AsyncClient, governance_type: Optional[GovernanceType.enum] = None, config_account_type:Optional[ConfigAccountType.enum] = None, vote_account_governance: Optional[VoteAccountGovernance.enum] = None,  log_level: int = 0) -> str:
    vote_account_pubkey, _vote_account_bump = PublicKey.find_program_address([bytes(ingl_constants.VOTE_ACCOUNT_KEY, 'UTF-8'), bytes(payer_keypair.public_key)], get_program_id())
    general_account_pubkey, _general_account_bump = PublicKey.find_program_address([bytes(ingl_constants.GENERAL_ACCOUNT_KEY, 'UTF-8'), bytes(payer_keypair.public_key)], get_program_id())
    config_account_pubkey, _config_account_bump = PublicKey.find_program_address([bytes(ingl_constants.CONFIG_ACCOUNT_KEY, 'UTF-8'), bytes(payer_keypair.public_key)], get_program_id())
    nft_account_data_pubkey, _nft_account_data_bump = PublicKey.find_program_address([bytes(ingl_constants.NFT_ACCOUNT_DATA_KEY, 'UTF-8'), bytes(mint.public_key)], get_program_id())
    associated_account_key = assoc_instructions.get_associated_token_address(payer_keypair.public_key, mint.public_key)
    ...
    general_account_meta = AccountMeta(pubkey = general_account_pubkey, is_signer = False, is_writable = True)
    mint_account_meta = AccountMeta(pubkey = mint.public_key, is_signer = False, is_writable = False)
    ...
    instruction_data = build_instruction(InstructionEnum.enum.InitGovernance(), governance_type=governance_type, config_account_type=config_account_type, vote_account_governance=vote_account_governance, log_level=log_level)
    
    if instruction_data[1] == 1:
        buffer_account_key = PublicKey(instruction_data[2:34])
...
async def vote_governance(payer_keypair: KeypairInput, vote: Bool, proposal_numeration: int, mints: List[PublicKey], client: AsyncClient, log_level: int = 0) -> str:
    ...
    for mint in mints:
        associated_account_key = assoc_instructions.get_associated_token_address(payer_keypair.public_key, mint)
        nft_account_pubkey, _nft_account_bump = PublicKey.find_program_address([bytes(ingl_constants.INGL_NFT_KEY, 'UTF-8'), bytes(mint)], get_program_id())
...
async def execute_governance(payer_keypair: KeypairInput, proposal_numeration: int, client: AsyncClient, log_level: int = 0) -> str:
    ...
    if governance_type == 0:
        ...
        buffer_account_key = PublicKey(proposal_data[loc:loc+32])
        programdata_key, _programdata_bump = PublicKey.find_program_address([bytes(get_program_id())], ingl_constants.BPF_LOADER_UPGRADEABLE)
        upgrade_authority_key, _upgrade_authority_bump = PublicKey.find_program_address([bytes(ingl_constants.INGL_PROGRAM_AUTHORITY_KEY, 'UTF-8'), bytes(get_program_id())], get_program_id())
        ...
        loc += 1
        vote_account_key, _vote_account_bump = PublicKey.find_program_address([bytes(ingl_constants.INGL_VOTE_KEY, 'UTF-8'),], get_program_id())
        authorized_withdrawer_key, _authorized_withdrawer_bump = PublicKey.find_program_address([bytes(ingl_constants.INGL_VOTE_AUTHORIZED_WITHDRAWER_KEY, 'UTF-8'),], get_program_id())
...
async def reset_uris(payer_keypair: KeypairInput, client: AsyncClient, log_level: int = 0) -> str :
    ...
    payer_account_meta = AccountMeta(pubkey = payer_keypair.public_key, is_signer = True, is_writable = True) # payer is the validator id address.
    uris_account_key, uris_account_bump = PublicKey.find_program_address([bytes(ingl_constants.INGL_URIS_ACCOUNT_KEY, 'UTF-8')], get_program_id())
